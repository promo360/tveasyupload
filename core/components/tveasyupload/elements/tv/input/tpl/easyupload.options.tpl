...
<div class="tveasyuploadInfo">
	...
	<p>Путь к сохраненному файлу и дополнительный префикс имени файла могут быть настроены динамически с несколькими заполнителями:
		<ul>
			...
			<li><span>{ra}</span>   - Алиас ресурса (ВНИМАНИЕ! алиас не должен содержать спецсиволов, запрещенных в ОС)</li>
			<li><span>{p}</span>    - ID ресурса родителя</li>
			<li><span>{pa}</span>   - Алиас ресурса родителя (ВНИМАНИЕ! алиас не должен содержать спецсиволов, запрещенных в ОС)</li>
